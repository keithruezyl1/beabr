...
function getOtpSentAtStorageKey(email) {
  return `beabr:login-otp-sent-at:${normalizeEmail(email)}`;
}

function readOtpSentAt(email) {
  if (typeof window === "undefined") return 0;
  const normalized = normalizeEmail(email);
  if (!normalized) return 0;
  const raw = window.localStorage.getItem(getOtpSentAtStorageKey(normalized));
  const parsed = Number(raw);
  return Number.isFinite(parsed) ? parsed : 0;
}

function writeOtpSentAt(email, timestamp) {
  if (typeof window === "undefined") return;
  const normalized = normalizeEmail(email);
  if (!normalized) return;
  window.localStorage.setItem(getOtpSentAtStorageKey(normalized), String(timestamp));
}

function clearOtpSentAt(email) {
  if (typeof window === "undefined") return;
  const normalized = normalizeEmail(email);
  if (!normalized) return;
  window.localStorage.removeItem(getOtpSentAtStorageKey(normalized));
}

function getOtpResendRemainingMs(email, now = Date.now()) {
  const sentAt = readOtpSentAt(email);
  if (!sentAt) return 0;
  return Math.max(0, sentAt + OTP_RESEND_COOLDOWN_MS - now);
}
...
export function LoginPage() {
  ...
  async function onSendCode(e) {
    e?.preventDefault?.();
    if (getOtpResendRemainingMs(email) > 0) return;
    setBusy(true);
    setStep("otp");
    try {
      await sendEmailOtp(email);
      writeOtpSentAt(email, Date.now());
      setClock(Date.now());
  ...
  async function onVerifyOtp(e) {
    ...
    try {
      ...
      clearOtpSentAt(email);
    } catch (e2) {
      const message = String(e2?.message || "").toLowerCase();
      const otpExpired = message.includes("expired") || message.includes("otp_expired");
      if (otpExpired) {
        const remaining = getOtpResendRemainingMs(email);
        setOtp("");
        setStep("email");
        setClock(Date.now());
        if (remaining > 0) {
          toast.caution(
            `That code expired. You can request a new one in ${formatCountdown(remaining)}.`
          );
        } else {
          toast.caution("That code expired. You can request a new one now.");
        }
      } else {
  ...
  return (
    <div className="fixed inset-0 flex w-screen items-center justify-center p-5">
      <img
        ...
        alt=""
        aria-hidden="true"
        className="pointer-events-none absolute bottom-0 left-0 h-[180px] w-auto select-none opacity-95 sm:h-[220px] md:h-[260px]"
      />
      <div className="flex w-full max-w-md flex-col items-center justify-center space-y-4 text-center">
        ...
        <Card className="w-full p-5">
          <form className="space-y-3" onSubmit={step === "email" ? onSendCode : onVerifyOtp}>
            ...
            <Button type="submit" className="w-full" disabled={busy || sendBlockedByCooldown}>
              {busy
                ? "Working..."
                : step === "email"
                  ? sendBlockedByCooldown
                    ? `Send code in ${formatCountdown(resendRemainingMs)}`
                    : "Send code"
                  : "Verify code"}
            </Button>
            ...
            <p className="pt-2 text-center text-[11px] leading-relaxed text-[var(--text-muted)]">
              ...
              <Link
                ...
                className="font-semibold text-[var(--color-primary-700)] underline decoration-[rgba(129,160,63,0.55)] underline-offset-2 hover:text-[var(--color-primary-800)]"
              >
                Terms of use
              </Link>{" "}
              and{" "}
